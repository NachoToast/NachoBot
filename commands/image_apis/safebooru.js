...
const blacklisted_tags = [
    'loli', 'beastiality', 'shota', 'rape', 'incest', 'child', 'nacho'
];
...
function scuffed_encode(str) {
    str = str.replace(/\s/g, "_").replace("(", "%28").replace(")", "%29").replace("!", "%21").replace(":", "%3A").replace("+", "%2B");
    return str;
}
    ...
    async execute(client, interaction) {
        ...
        if (interaction.options.length > 0) {
            ...
            if (interaction.options.length > 1) {
                if (blacklisted_tags.includes(interaction.options[1].value.toLowerCase())) {
                    interaction.reply(`Seriously? 🤮`, { ephemeral: true });
                    return;
                }
        ...
        if (response.length == 0 || response[0]?.file_url === undefined) {
            interaction.editReply(notfound_messages[Math.floor(Math.random() * notfound_messages.length)]);
            return;
        }
    ...
    async execute_help(client, interaction) {
        if (interaction.options.length > 1) {
            if (interaction.options[1].value === 'tags') {
                interaction.reply(`Specify tags the same way the site specifies, <https://safebooru.donmai.us/wiki_pages/help%3Acheatsheet>, maximum of 2 and some are blacklisted, so don't even think about it. 🤨\nIf you can't be fucked reading that, basically just put the character name of the person you want and if that doesn't work add a space and an asterisk to the end.`);
                return;
            }
        }
        await interaction.reply(`Gets a random post from the Safebooru API.`, { ephemeral: true });
    }
}