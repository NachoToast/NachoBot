...
module.exports = {
  name: 'san',
  aliases: ['s', 'safebooru', 'safe'],
  async execute(client, interaction) {
    if (!interaction.channel.nsfw) {
      interaction.reply(
        `Take it to NSFW you sick freak (yes this still applies to Safebooru).`,
        { ephemeral: true }
      );
      return;
    }
  ...
  async execute_help(client, interaction) {
    if (interaction.options.length > 1) {
      if (interaction.options[1].value === 'tags') {
        interaction.reply(
          `Specify tags the same way the site specifies, <https://safebooru.donmai.us/wiki_pages/help%3Acheatsheet>, maximum of 2 and some are blacklisted, so don't even think about it. 🤨\nIf you can't be fucked reading that, basically just put the character name of the person you want and if that doesn't work add a space and an asterisk to the end.`
        );
        return;
      }
    }
    await interaction.reply(`Gets a random post from the Safebooru API.`, {
      ephemeral: true,
    });
  },
};
