...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    if (!interaction.channel.nsfw) {
      interaction.reply(`Please use this command in a NSFW channel.`, {
        ephemeral: true,
      });
      return;
    }
    ...
    if (io_map.indexOf('tags') !== -1) {
      let tag_array =
        interaction.options[io_map.indexOf('tags')].value.split(',');
    ...
    if (
      io_map.indexOf('meta') !== -1 &&
      interaction.options[io_map.indexOf('meta')].value === true
    )
      var verbose = true;
    ...
    try {
      ...
      interaction.reply(
        `Error occurred connecting to the API. Try again in: *273 days, 5 hours, and 19 minutes*.`,
        { ephemeral: true }
      );
      console.log(
        `${new Date().toLocaleString()}: Error connecting to NachoToast weeb API.`
      );
      return;
    }
    if (data === undefined || data?.length === 0 || typeof data !== 'object') {
      ...
      console.log(
        `${new Date().toLocaleString()}: Error getting result from NachoToast weeb API.`
      );
      return;
    }
    ...
    setTimeout(function () {
      interaction.editReply(
        `Tags: ${tags} (${data.size}KB)\nSource: ${data.source}`
      );
      if (verbose) {
        let metadata = `Uploaded ${(
          (new Date().getTime() / 1000 - data.added) /
          86400
        ).toFixed(0)} days ago by ${
          data.uploader
        }.\nNachoToast.com took ${data.took.toFixed(
          3
        )}s to deliver, Discord took ${(
          (new Date().getTime() - start - 1500) /
          1000
        ).toFixed(3)}s`;
      ...
      if (data?.tag_info) {
        ...
        for (let i = 0, len = data.tag_info.length; i < len; i++) {
          if (data.tag_info[i].status == 'Guessed')
            output += `\nI'm ${data.tag_info[i].confidence}% sure you meant '${data.tag_info[i].tag}'`;
          else if (data.tag_info[i].status == 'Unknown')
            output += `\Couldn't find a matching tag for ${data.tag_info[i].tag}`;
          else if (data.tag_info[i].status == 'Indirect Alias')
